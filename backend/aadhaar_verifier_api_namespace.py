...
aadhaar_verifier_api_ns = Namespace(name='aadhaar_verifier_api_ns', description='Aadhaar Verifier API Namespace', path='/')
...
class UIDAIGenerateOTP(Resource):
    ...
    @aadhaar_verifier_api_ns.response(code=200, description='OK', model=uidai_generate_otp_api_response_model)
    @aadhaar_verifier_api_ns.response(code=400, description='BAD REQUEST', model=task_api_400_error_response_model)
    @aadhaar_verifier_api_ns.response(code=422, description='UNPROCESSABLE ENTITY', model=task_api_422_error_response_model)
    @aadhaar_verifier_api_ns.response(code=500, description='INTERNAL SERVER ERROR', model=task_api_500_error_response_model)
    @timeit
    def post(self):
        try:
            ...
            return e.task_api_error_response_model, e.task_api_error_response_model.get("code")
            ...
            return task_api_error_response_model, task_api_error_response_model.get("code")
...
class UIDAIVerifyAadhaar(Resource):
    ...
    @aadhaar_verifier_api_ns.response(code=200, description='OK', model=uidai_verify_aadhaar_api_response_model)
    @aadhaar_verifier_api_ns.response(code=400, description='BAD REQUEST', model=task_api_400_error_response_model)
    @aadhaar_verifier_api_ns.response(code=422, description='UNPROCESSABLE ENTITY', model=task_api_422_error_response_model)
    @aadhaar_verifier_api_ns.response(code=500, description='INTERNAL SERVER ERROR', model=task_api_500_error_response_model)
    @timeit
